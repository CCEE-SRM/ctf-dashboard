                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="lg:col-span-1">
                        <div className="bg-white border-4 border-black p-6 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] sticky top-8">
                            <h2 className="text-2xl font-pixel mb-6 border-b-2 border-black pb-2">ADD ADMIN ACCOUNT</h2>
                            <form onSubmit={handleAddAdmin} className="space-y-4">
                                ...
                                >
                                    {isAddingAdmin ? 'AUTHORIZING...' : 'AUTHORIZE ADMIN'}
                                </button>
                                <p className="text-xs text-zinc-500 font-mono italic">
                                    NOTE: If the user does not exist, a placeholder account will be created. They will gain admin access upon their first login.
                                </p>
                            </form>
                        </div>
                    </div>
                                    ...
                                    <tbody>
                                        {filteredUsers.map((user) => (
                                            <tr key={user.id} className="border-b border-zinc-200 hover:bg-zinc-50 transition-colors">
                                                ...
                                                <td className="p-3">
                                                    <span className={`px-2 py-0.5 text-[10px] font-pixel ${user.role === 'ADMIN' ? 'bg-purple-100 text-purple-700 border border-purple-300' : 'bg-zinc-100 text-zinc-500 border border-zinc-200'}`}>
                                                        {user.role}
                                                    </span>
                                                </td>
                                                <td className="p-3 text-right">
                                                    {user.id !== dbUser.id ? (
                                                        <button
                                                            ...
                                                            disabled={!!updating}
                                                            className={`text-[10px] font-pixel px-2 py-1 border border-black shadow-[2px_2px_0px_0px_#000] hover:translate-y-[1px] hover:shadow-[1px_1px_0px_0px_#000] transition-all
                                                                ${user.role === 'ADMIN' ? 'bg-red-300 text-red-800' : 'bg-retro-green text-black'}`}
                                                        >
                                                            {updating === user.id ? '...' : user.role === 'ADMIN' ? 'REVOKE' : 'PROMOTE'}
                                                        </button>
                                                    ) : (
